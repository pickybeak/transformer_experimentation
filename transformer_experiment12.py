...
def program_loop():
    ...
    class PositionAttentionLayer(pl.LightningModule):
        def __init__(self, d_p, d_v, d_model, dropout_ratio, max_length=100, k=hparams.k):
            ...
            self.max_length = max_length
            self.d_model = d_model
            self.overall_transform = nn.Linear(self.d_model, self.d_v)
        ...
        def forward(self, x, query_len):
            ...
            # overall : [batch_size, query_len, d_v]
            ...
            '''
            abs_pos = abs_pos.permute(1,0)
            similarity = torch.matmul(overall, abs_pos)
            abs_pos = abs_pos.permute(1, 0)
